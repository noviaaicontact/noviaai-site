...
function hoursLookConfigured(hours) {
  const raw = (hours && hours.horaire) || hours || {};
  return Object.values(raw).some((d) => (
    d && typeof d === 'object' && (d.ouvert === true || d.ouvert === 'true' || d.debut)
  ));
}

/** Ouvert maintenant, fuseau America/Toronto. Fin exclusive (17h = fermé à 17:00). */
function isOpenNow(hours, at = new Date()) {
  if (!hoursLookConfigured(hours)) return false;
  const normalized = normalizeHours(hours);
  const parts = torontoParts(at);
  const row = normalized[DAY_KEYS[parts.weekday]];
  if (!row || row.ouvert === false || row.ouvert === 'false') return false;
  const start = parseHHMM(row.debut);
  const end = parseHHMM(row.fin);
...
const WISH_RE = /\b(je voudrais|j['’]aimerais|je veux|est[- ]ce que|auriez[- ]vous|avez[- ]vous|c['’]est possible|etes[- ]vous (libre|ouvert)|êtes[- ]vous (libre|ouvert))\b/i;
const SLOT_REJECT_RE = /d[ée]j[àa] pris|pas (libre|disponible)|n['’]est pas libre|ne (peux|peut) pas confirmer|cette heure n['’]est pas|cr[ée]neau (n['’]est )?pas|heure (demand[ée]e )?n['’]est pas/i;

function assistantRejectedSlot(text) {
  return SLOT_REJECT_RE.test(String(text || ''));
}

function userClearlyAccepted(text) {
  return ACCEPT_RE.test(String(text || ''));
}

const CALENDAR_CLAIM_RE = /confirm[ée]e?|inscrit[e]? à l['’]agenda|not[ée]e? au calendrier|d[ée]j[àa] pris|n['’]est pas libre|pas (libre|disponible)|cr[ée]neau|je (peux|vais) vous proposer|voici (les )?(disponibilit|plages)|quelle option vous convient|c['’]est dans l['’]agenda|je (viens de )?r[ée]serv/i;

/** Enlève ce que l'IA a inventé sur l'agenda. Les faits viennent du backend. */
function stripCalendarClaims(text) {
  ...
  for (const chunk of chunks) {
    ...
    if (!t) continue;
    if (/^[-•*]\s*/.test(t) && /\d{1,2}\s*h/i.test(t)) continue;
    if (CALENDAR_CLAIM_RE.test(t)) continue;
    kept.push(t);
  }
...
function extractAcceptedSlot({ userMessage, aiReply, qualificationData, history, hours, durationMin }) {
  ...
  const asking = WISH_RE.test(user) && !userClearlyAccepted(user);
  if (parseHHMM(userNorm) && !asking) {
