...
const MAX_SLOTS = 14;
const SCHEDULING_RE = /rendez-vous|rendez vous|rdv|réserver|reserver|reservation|réservation|disponib|horaire|plage|créneau|creneau|calendrier|calendar|agenda|google|outlook|demain|aujourd|lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche|\d{1,2}\s*h\b|\d{1,2}:\d{2}/i;
...
function parseAcceptedSlot(text, hours, fromDate = new Date()) {
  ...
  if (iso) {
    ...
    return {
      ...
      end: new Date(start.getTime() + SLOT_MINUTES * 60 * 1000).toISOString(),
    };
  }
...
function parseSlotFromLastTimedLine(text, hours) {
  const lines = String(text || '').split(/\n/).map((l) => l.trim()).filter(Boolean);
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    if (!parseHHMM(lines[i].toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''))) continue;
    const slot = parseAcceptedSlot(lines[i], hours);
    if (slot) return slot;
  }
  return parseAcceptedSlot(text, hours);
}

/** Préfère l’heure dite par le client, pas la liste de plages de l’agent. */
function extractAcceptedSlot({ userMessage, aiReply, qualificationData, history, hours }) {
  const fromQual = parseAcceptedSlot(qualificationData?.creneau_confirme || '', hours);
  ...
  if (parseHHMM(userNorm)) {
    const slot = parseAcceptedSlot(user, hours);
    if (slot) return slot;
  }

  if (ACCEPT_RE.test(user)) {
    const lastAsst = [...(history || [])].reverse().find((m) => m.role === 'assistant');
    const fromAi = parseSlotFromLastTimedLine(aiReply || (lastAsst && lastAsst.content) || '', hours);
