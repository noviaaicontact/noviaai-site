...
async function formatAvailabilityForPrompt(tenantId, dossier) {
  ...
  if (!result || result.timeout) {
    ...
    if (!connected) return '';
    return '\n\nAGENDA RÉEL (calendrier connecté)\nLe calendrier est connecté, mais les plages n\'ont pas chargé à temps. Dis que tu as accès à l\'agenda. Propose selon les HORAIRES, sans inventer de conflit. Si le client choisit une heure, confirme-la.\n';
  }
  ...
  if (!result.slots.length) {
    return '\n\nAGENDA RÉEL (calendrier Google/Outlook CONNECTÉ)\nAucune plage libre dans les 7 prochains jours selon l\'agenda. Propose de noter les disponibilités pour un rappel humain. N\'invente aucune heure. Si on te demande si tu as accès au calendrier : oui.\n';
  }
...
- Si le client demande une heure QUI EST dans la liste : confirme-la (jour + heure). Ne dis PAS que tu transmets à l'équipe, ne dis PAS que tu ne peux pas confirmer.
- Si le client demande une heure QUI N'EST PAS dans la liste : dis que c'est déjà pris et propose 2–3 plages de la liste.
Ne dis JAMAIS « je ne peux pas confirmer d'heure précise ».
${lines.join('\n')}\n`;
}
