...
/** Jours d'essai Stripe restants — 0 si l'essai local est déjà consommé/expiré. */
function stripeTrialDays(tenant) {
...
async function createCheckoutSession({ tenant, plan, successUrl, cancelUrl }) {
  ...
  const trialDays = stripeTrialDays(tenant);
  const params = {
    mode: 'subscription',
    customer: customerId,
    ...
    subscription_data: {
      // Pas de 2e essai Stripe si l'essai local de 14 j est déjà passé.
