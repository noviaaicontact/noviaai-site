...
const { saveThreadQualification, loadThreadQualification } = require('./qualification');

function buildVoicemailSmsFallback(extracted, agentName) {
  const agent = agentName || 'notre équipe';
  const raison = String((extracted && extracted.raison) || '').trim();
  if (raison) {
    return `Ici ${agent}. J'ai écouté votre message (${raison.slice(0, 120)}). Répondez ici, je peux vous aider.`;
  }
  return `Ici ${agent}. J'ai écouté votre message. Répondez ici, je peux vous aider.`;
}

async function replyToVoicemailBySms({ tenant, callerPhone, twilioNumber, transcript, extracted }) {
  const text = String(transcript || '').trim();
  if (!tenant?.id || !callerPhone || !twilioNumber || text.length < 8) return null;
  if (!isActive(tenant)) return null;

  const { isOptedOut, appendPromoFooter } = require('./sms-compliance');
  if (await isOptedOut(tenant.id, callerPhone)) return null;
  ...
  const { processInboundActions } = require('./agent-tools');

  const dossier = rowToDossier(tenant);
  const agentName = (dossier.identite_agent && dossier.identite_agent.nom_agent) || tenant.agent_name || 'notre équipe';
  const key = convoKey(twilioNumber, callerPhone);
  ...
  const userMessage = extracted?.nom
    ? `Je m'appelle ${extracted.nom}. ${text}`
    : text;

  let reply = null;
  try {
    reply = await generateReply(dossier, history, userMessage, tenant.id, {
      voicemailReply: true,
      qualificationData,
    });
  } catch (e) {
    console.error('voicemail generateReply', e.message);
  }
  if (!reply) reply = buildVoicemailSmsFallback(extracted, agentName);

  const body = appendPromoFooter(reply, tenant.business_name);
  await sendSMS({ to: callerPhone, from: twilioNumber, body });

  const next = [
    ...(history || []),
    { role: 'user', content: `Message vocal : ${text}` },
    { role: 'assistant', content: body },
  ];
  await saveHistory(key, next, tenant.id, callerPhone);
  await logMessage(tenant.id, callerPhone, 'outbound', body);
  await touchThread(tenant.id, callerPhone, body, 'open');
  await logEvent(tenant.id, callerPhone, 'sms_outbound', {
    body: body.slice(0, 160),
    trigger: 'voicemail_reply',
  });

  try {
    await processInboundActions({
      tenant,
      callerPhone,
      userMessage: text,
      aiReply: body,
    ...
    console.error('voicemail reply actions', e.message);
  }
...
async function processVoicemailRecording({
  ...
  if (transcript && transcript.trim().length >= 8) {
    try {
      smsReply = await replyToVoicemailBySms({
        tenant,
        callerPhone,
        twilioNumber,
        transcript,
      ...
      console.error('voicemail sms reply', e.message);
    }
  }

  return { transcript, extracted, body, smsReply };
}
...
module.exports = {
  ...
  buildVoicemailTwiml,
  buildVoicemailSmsFallback,
  replyToVoicemailBySms,
};
