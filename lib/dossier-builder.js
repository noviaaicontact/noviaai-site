...
function rowToDossier(row) {
  ...
  const publicPhone = (row.provisioning_status === 'active' && row.twilio_number)
    ? row.twilio_number
  ...
  return {
    ...
    scripts: {
      ...
      reservation: row.reservation_url
        ? `Lien de réservation : ${row.reservation_url}`
        : 'Prendre nom + disponibilités → demander_rendez_vous',
...
function formToTenantPayload(body) {
  ...
  const lineMode = body.line_mode === 'hosted' ? 'hosted' : 'new';
  const payload = {
    business_name: body.business_name || 'Mon commerce',
    business_type: body.business_type || 'PME',
    agent_name: body.agent_name || 'Léa',
    agent_tone: body.agent_tone || '',
    phone_forward: phone,
    existing_business_number: lineMode === 'hosted'
    ...
    hours: body.hours || DEFAULT_HOURS,
