...
const { normalizeQualificationFields } = require('./qualification');
...
function rowToDossier(row) {
  ...
  const qualificationFields = normalizeQualificationFields(row.qualification_fields);
  ...
  return {
    ...
    identite_agent: {
      nom_agent: agentName,
      role: `Employé virtuel de ${businessName} — tu représentes l'entreprise à l'accueil`,
...
function formToTenantPayload(body) {
  ...
  const payload = withChatbotDefaults({
    ...
    qualification_fields: normalizeQualificationFields(body.qualification_fields),
...
function settingsToTenantPayload(body, existing) {
  ...
  if (body.qualification_fields !== undefined) {
    merged.qualification_fields = normalizeQualificationFields(body.qualification_fields);
  }
