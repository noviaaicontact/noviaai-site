...
function defaultMissedCallSms(businessName, agentName, businessType) {
  const workflow = detectQualificationWorkflow({
    business_name: businessName,
    business_type: businessType,
  });
  return missedCallSmsForWorkflow(workflow, businessName, agentName);
}
...
function getChatbotStarterKit(opts = {}) {
  ...
  const businessType = opts.business_type || opts.businessType || '';
  const workflow = detectQualificationWorkflow({ business_name: businessName, business_type: businessType });
  return {
    agent_name: agentName,
    agent_tone: DEFAULT_AGENT_TONE,
    welcome_sms: defaultWelcomeSms(businessName, agentName),
    missed_call_sms: defaultMissedCallSms(businessName, agentName, businessType),
    qualification_fields: defaultFieldsForWorkflow(workflow),
    hours: { ...DEFAULT_HOURS },
  };
}
...
function withChatbotDefaults(payload) {
  ...
  const starter = getChatbotStarterKit({
    businessName,
    agentName,
  ...
  if (!Array.isArray(p.agent_favorites)) p.agent_favorites = [];
  if (!Array.isArray(p.qualification_fields) || !p.qualification_fields.length) {
    p.qualification_fields = starter.qualification_fields;
  }
  return p;
}
