...
const SKIP_URL_RE = /\/(wp-admin|wp-login|cart|checkout|panier|login|connexion|account|mon-compte|cdn-cgi|privacy|confidentialite|cookie|terms|conditions|mentions-legales|tag\/|category\/|author\/|feed\/|\#)/i;
...
function normalizeUrl(raw, base) {
  try {
    ...
    u.hash = '';
    // Retirer query tracking inutiles
...
async function fetchPageRaw(url, timeoutMs = 15000) {
  ...
  return { html, finalUrl: res.url || parsed.href, parsed };
}
...
function extractLinks(html, baseUrl) {
  ...
  $('a[href]').each((_, el) => {
    ...
    if (SKIP_URL_RE.test(abs)) return;
  ...
  for (const sm of candidates) {
    try {
      ...
      locs.forEach((loc) => {
        ...
        if (abs && sameSite(abs, origin) && !SKIP_URL_RE.test(abs)) found.push(abs);
      });
...
async function discoverSiteUrls(startUrl, maxPages = MAX_SITE_PAGES) {
  ...
  const add = (url, bonus = 0) => {
    ...
    if (!abs || !sameSite(abs, origin) || SKIP_URL_RE.test(abs)) return;
...
async function ingestWebsite(tenantId, startUrl, opts = {}) {
  ...
  const queue = await discoverSiteUrls(start, maxPages);
