...
async function ingestWebsite(tenantId, startUrl, opts = {}) {
  ...
  while (queue.length && pages.length < maxPages) {
    ...
    try {
      ...
      // url est déjà dans visited : ne pas se skipper soi-même si canon === url
      if (canon !== url && visited.has(canon)) continue;
      if (pages.some((p) => p.finalUrl === canon || p.finalUrl === page.finalUrl)) continue;
      visited.add(canon);
