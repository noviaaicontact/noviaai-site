...
const FIELD_DEFINITIONS = {
  ...
  preferences: { label: 'Préférences du client', enabled: true, required: false },
  disponibilites: { label: 'Disponibilités', enabled: true, required: true },
  creneau_confirme: { label: 'Créneau souhaité', enabled: true, required: false },
  demande: { label: 'Demande / sujet', enabled: true, required: true },
  probleme: { label: 'Type de problème', enabled: true, required: true },
  urgence: { label: 'Urgence', enabled: true, required: true },
  depuis_quand: { label: 'Depuis quand', enabled: true, required: false },
...
3. Est-ce une urgence? (oui/non + contexte)
4. Depuis quand le problème existe
5. Adresse ou secteur pour l'intervention
6. Meilleur moment pour un rappel
