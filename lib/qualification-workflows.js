...
const APPOINTMENT_KEYWORDS = [
  ...
  'gym', 'entraînement', 'entrainement', 'entraineur', 'entraîneur', 'fitness',
  'conditionnement physique', 'musculation', 'crossfit', 'yoga', 'pilates',
  'studio', 'centre sportif', 'kinésiologue', 'kinesiologue', 'nutritionniste',
  'école', 'ecole', 'cours', 'formation', 'garderie',
];
...
const FIELD_SERVICE_KEYWORDS = [
  ...
  'piscine', 'hors-terre', 'creusée', 'creusee',
...
function detectQualificationWorkflow(tenantOrDossier) {
  ...
  // « Spas et Piscines » n'est pas un spa esthétique — sinon le workflow RDV (coupe, coiffeur) s'active.
  const isPoolRetail = matchesKeywords(blob, [
    'piscine', 'pool', 'thermopompe', 'hors-terre', 'hors terre', 'creusee', 'creusée',
  ]);
  if (isPoolRetail) return WORKFLOW_FIELD_SERVICE;
...
function missedCallSmsForWorkflow(_workflow, businessName, agentName) {
  const biz = businessName || 'notre entreprise';
  const agent = agentName || 'Léa';
  return (
    `Bonjour! Ici ${agent}, de ${biz}. Nous avons manqué votre appel. `
