// Envoi de SMS sortants (utilisé par le rattrapeur d'appels manqués).
const { getClientByNumber } = require('./clients');
const { saveHistory } = require('./store');
const { logMessage, logMissedCall } = require('./tenant');
const { convoKey } = require('./twilio-util');
const { logEvent } = require('./events');
const { touchThread } = require('./inbox');
const { isOptedOut, appendPromoFooter } = require('./sms-compliance');
const { checkSmsQuota } = require('./usage-limits');

function textbackMessage(dossier) {
  const s = (dossier && dossier.scripts) || {};
  const agent = (dossier && dossier.identite_agent && dossier.identite_agent.nom_agent) || 'notre équipe';
  return s.texto_rappel || s.accueil ||
    `Bonjour! Ici ${agent}. Nous avons manqué votre appel. Comment puis-je vous aider?`;
}

async function sendTextback(businessNumber, callerNumber) {
  const client = await getClientByNumber(businessNumber);
  const tenantId = client && client.tenant && client.tenant.id;
  if (!client || !client.tenant) {
    console.error('textback: tenant introuvable pour', businessNumber);
    return;
  }
  if (client.suspended) {
    console.warn('textback skipped — abonnement inactif', tenantId);
    await logMissedCall(tenantId, callerNumber, false);
    await logEvent(tenantId, callerNumber, 'missed_call', { textback: false, suspended: true });
    return;
  }
  const quota = await checkSmsQuota(client.tenant);
  if (!quota.ok) {
    console.warn('textback skipped — quota SMS', tenantId, quota.count, '/', quota.limit);
    await logMissedCall(tenantId, callerNumber, false);
    await logEvent(tenantId, callerNumber, 'missed_call', { textback: false, quota_exceeded: true });
    const { notifyAdminClientError } = require('./admin-alert');
    notifyAdminClientError({
      area: 'quota',
      error: `Limite SMS atteinte au rattrapage (${quota.count}/${quota.limit})`,
      tenant: client.tenant,
      extra: { count: quota.count, limit: quota.limit, callerNumber },
      maxPerHour: 1,
    }).catch(() => {});
    return;
  }
  if (await isOptedOut(tenantId, callerNumber)) {
    console.warn('textback skipped — opted out', callerNumber);
    await logMissedCall(tenantId, callerNumber, false);
    await logEvent(tenantId, callerNumber, 'missed_call', { textback: false, opted_out: true });
    return;
  }
  const businessName = (client.tenant && client.tenant.business_name) || '';
  const msg = appendPromoFooter(textbackMessage(client.dossier), businessName);
  let textbackOk = false;
  let smsError = null;
  try {
    await sendSMS({ to: callerNumber, from: businessNumber, body: msg });
    textbackOk = true;
  } catch (e) {
    smsError = e;
    console.error('textback SMS failed', {
      to: callerNumber,
      from: businessNumber,
      error: e.message,
      code: e.code,
    });
  }

  if (tenantId) {
    await logMissedCall(tenantId, callerNumber, textbackOk);
    if (textbackOk) {
      const key = convoKey(businessNumber, callerNumber);
      await saveHistory(key, [{ role: 'assistant', content: msg }], tenantId, callerNumber);
      await logMessage(tenantId, callerNumber, 'outbound', msg);
      await logEvent(tenantId, callerNumber, 'missed_call', { textback: true });
      await logEvent(tenantId, callerNumber, 'sms_outbound', { body: msg.slice(0, 160), trigger: 'missed_call' });
      await touchThread(tenantId, callerNumber, msg, 'open');
      const { scheduleMissedCallFollowup } = require('./followup');
      await scheduleMissedCallFollowup(client.tenant, callerNumber);
      // Pas de notif proprio ici : Camille gère. Courriel seulement
      // si l'agent juge que c'est un dossier payant ou un client mécontent.
    } else if (smsError) {
      await logEvent(tenantId, callerNumber, 'missed_call', {
        textback: false,
        error: smsError.message,
        code: smsError.code || null,
      });
    }
  }

  if (smsError) throw smsError;
}

async function sendSMS({ to, from, body }) {
  const sid = process.env.TWILIO_ACCOUNT_SID;
  const token = process.env.TWILIO_AUTH_TOKEN;
  if (!sid || !token) throw new Error('Twilio non configuré (TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN)');
  const twilio = require('twilio');
  const c = twilio(sid, token);
  return c.messages.create({ to, from, body });
}

module.exports = { sendTextback, sendSMS, textbackMessage };
