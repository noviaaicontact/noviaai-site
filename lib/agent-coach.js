...
function parseCoachDecision(raw) {
  ...
  try {
    ...
    return {
      ...
      instructions_add: String(data.instructions_add || '').trim().slice(0, 800),
      favorites_add: favorites.slice(0, 5),
      services_upsert: services.slice(0, 12),
      links_add: links.slice(0, 6),
      faq_add: faq.slice(0, 6),
      workflow: wf,
...
Méthode :
1. Écoute comment ÇA MARCHE chez eux.
2. Crée / mets à jour les services (nom + booking_mode + url si lien + notify_owner).
3. Pose UNE question s'il manque l'action d'un service important.
4. Tutoiement, court, québécois. Confirme ce que tu as enregistré.
