...
function validateCapture(body) {
  ...
  const emailRaw = text(body.email, 160).toLowerCase();
  ...
  if (emailRaw && (!isEmail(emailRaw) || looksFakeEmail(emailRaw))) errors.push('courriel');
  if (!phone) errors.push('téléphone');
  if (inbound && !INBOUND_CHANNELS.includes(inbound)) errors.push('réception des demandes');
  if (missedCalls && !E.MISSED_CALLS[missedCalls]) errors.push('appels manqués');
  if (clientValue && !E.CLIENT_VALUE[clientValue]) errors.push('valeur d’un job');
  ...
  return {
    ...
    lead: {
      ...
      phone,
      email: emailRaw || '',
      inbound_channel: INBOUND_CHANNELS.includes(inbound) ? inbound : null,
      missed_calls_per_month: missedCalls || null,
      avg_client_value: clientValue || null,
      estimated_recovery_monthly: estimate.valid ? estimate.monthly : null,
