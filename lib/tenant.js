...
const { TRIAL_PLAN } = require('./plans');
...
const USER_PATCHABLE_FIELDS = new Set([
  ...
  'website_url', 'avg_client_value', 'hours', 'services', 'faq', 'policies', 'agent_favorites',
  'qualification_fields', 'qualification_workflow',
  'onboarding_done', 'notify_email', 'dossier',
...
async function getTenantById(tenantId) {
  if (!tenantId || !isDbConfigured()) return null;
  const db = getAdmin();
...
async function createTenantForUser(user, opts = {}) {
  ...
  // Essai gratuit = Essentiel seulement (opts.plan ignoré volontairement).
  ...
  const row = {
    ...
    policies: [],
    agent_favorites: [],
    qualification_fields: starter.qualification_fields || [],
  };
...
async function applyTenantPatch(column, value, patch) {
  const db = getAdmin();
  const safe = pickPatch(patch, USER_PATCHABLE_FIELDS);
  delete safe.plan;
  delete safe.subscription_status;
  if (!Object.keys(safe).length) throw new Error('Aucun champ modifiable');
  const { data: current } = await db.from('tenants').select('*').eq(column, value).maybeSingle();
  const merged = { ...(current || {}), ...safe };
  if (safe.onboarding_done === true && !current?.onboarding_done) {
    const ready = validateOnboarding(merged);
    if (!ready.ok) {
      const err = new Error(ready.errors.join(' '));
  ...
  const { data, error } = await db.from('tenants').update(merged).eq(column, value).select('*').single();
  if (error) throw error;
  return data;
}

async function updateTenant(userId, patch) {
  return applyTenantPatch('user_id', userId, patch);
}

async function updateTenantById(tenantId, patch) {
  return applyTenantPatch('id', tenantId, patch);
}
...
async function logMessage(tenantId, callerPhone, direction, body) {
  ...
  const db = getAdmin();
  const phone = toE164(callerPhone) || callerPhone;
...
async function resolveClient(twilioNumber) {
  ...
  if (raw) {
    // suspended = abo hors trialing/active → voix + SMS agent coupés
    // (opt-out LCAP reste géré même suspendu).
    const suspended = !isActive(raw);
    return {
      key: raw.id,
      tenant: raw,
      dossier: rowToDossier(raw),
      suspended,
    };
  }
