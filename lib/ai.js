...
async function generateReply(dossier, history, userMessage, tenantId) {
  ...
  if (tenantId) {
    try {
      const { looksLikeScheduling, formatAvailabilityForPrompt } = require('./calendar');
      if (looksLikeScheduling(userMessage)) {
  ...
  const messages = [
    { role: 'system', content: buildSystemPrompt(dossierForPrompt) + kbContext + calendarContext },
