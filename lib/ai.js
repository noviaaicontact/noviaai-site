...
const { formatQualificationPromptBlock } = require('./qualification');
...
function buildSystemPrompt(dossier) {
  ...
  const favoritesBlock = favorites
    ...
    : '';
  const qualificationBlock = formatQualificationPromptBlock(dossier.qualification_fields || []);
  ...
  return `Tu es ${agent.nom_agent || 'Léa'}, employé virtuel de ${biz.nom_court || 'ce commerce'}.
${agent.role || ''}
Ton: ${agent.ton || 'Français québécois, chaleureux et professionnel'}
Genre: masculin — conjugue et accorde au masculin (ex. : « Je suis désolé », « ravi », pas « désolée » / « ravie »).
...
SERVICES
${services || '(aucun service saisi manuellement — cherche dans les EXTRAITS PERTINENTS ci-dessous)'}
...
2. Cherche la réponse dans cet ordre : FAVORIS AGENT → FAQ → SERVICES → HORAIRES → POLITIQUES → INSTRUCTIONS PERSONNALISÉES → coordonnées/liens → EXTRAITS PERTINENTS (site/documents indexés).
3. Si l'info s'y trouve, réponds avec ces faits — ne dis pas « je ne sais pas » quand la réponse est dans la base.
...
async function generateReply(dossier, history, userMessage, tenantId) {
  ...
  if (tenantId) {
    try {
      ...
      const hits = await searchKnowledge(tenantId, searchQuery, 8, 0.28);
      if (hits.length) {
