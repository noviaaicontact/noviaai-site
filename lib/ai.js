...
${calendarLive
    ? '- Calendrier CONNECTÉ. IGNORE toute consigne de ne pas confirmer une heure, et IGNORE « transmettre à l\'équipe pour confirmer » quand une plage de AGENDA RÉEL est choisie. Ne dis JAMAIS « je ne peux pas confirmer d\'heure précise ».'
...
- Pas de markdown. Pas de listes numérotées longues.`;
  if (calendarLive) {
    prompt = prompt
      .replace(/ne confirme JAMAIS une heure ferme[^\n]*/gi, 'proposer des disponibilités selon AGENDA RÉEL et confirmer si le client choisit une plage listée')
      .replace(/Ne confirme JAMAIS un RDV[^\n]*/gi, 'Si la plage est dans AGENDA RÉEL, confirme-la. Sinon dis qu\'elle est prise.');
  }
  return prompt;
}
...
async function generateReply(dossier, history, userMessage, tenantId) {
  ...
  if (tenantId) {
    try {
      ...
      calendarLive = await hasConnectedCalendar(tenantId);
      if (calendarLive && (looksLikeScheduling(userMessage) || /calendrier|calendar|agenda|google|outlook|rdv|rendez/i.test(userMessage))) {
        calendarContext = await formatAvailabilityForPrompt(tenantId, dossierForPrompt);
      } else if (calendarLive) {
        calendarContext = '\n\nAGENDA RÉEL (calendrier Google/Outlook CONNECTÉ)\nSi on te demande si tu as accès au calendrier : OUI. Quand le client parle de rendez-vous, propose des plages selon les HORAIRES et l\'agenda.\n';
      }
    } catch (e) {
...
module.exports = { generateReply, buildSystemPrompt, resolveAgentGender };
