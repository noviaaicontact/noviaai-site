...
function buildSystemPrompt(dossier, opts = {}) {
  ...
  const voicemailReply = !!opts.voicemailReply;
...
${voicemailReply ? `
MESSAGE VOCAL
Le dernier message du client est un message vocal transcrit. Réponds comme s'il venait de t'écrire ça. Dis brièvement que tu as écouté le message. Ne dis pas que tu es une IA.
` : ''}
RÈGLES STRICTES
${rules}
- Réponses COURTES pour SMS (2–4 phrases max, ~${MAX_SMS} caractères).
${calendarLive
    ? '- Calendrier CONNECTÉ. N\'invente aucune heure. Ne dis JAMAIS qu\'un créneau est libre, pris, confirmé ou réservé — le système ajoutera les disponibilités et la confirmation.'
...
async function generateReply(dossier, history, userMessage, tenantId, opts = {}) {
  ...
  const messages = [
    { role: 'system', content: buildSystemPrompt(dossierForPrompt, { calendarLive, voicemailReply: !!opts.voicemailReply }) + kbContext + calendarContext },
