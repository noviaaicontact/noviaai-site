/** Session widget (`web:…`) ou simulateur (`test:…`) — pas un numéro réel. */
function isSyntheticCaller(raw) {
  return /^(web:|test:)/i.test(String(raw || '').trim());
}

/** Tests internes (simulateur dashboard / sessions `w_test_`) — pas un vrai client. */
function isTestCaller(raw) {
  const p = String(raw || '').trim();
  return /^test:/i.test(p) || /(?:^web:)?w_test_/i.test(p);
}

/** Normalise un numéro nord-américain en E.164 (+1XXXXXXXXXX). */
function toE164(raw) {
  const s = String(raw || '').trim();
  if (!s || isSyntheticCaller(s)) return null;
