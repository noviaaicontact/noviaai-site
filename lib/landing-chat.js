...
PRIX
- Forfait Pro unique : 199 $ CAD / mois (prix fondateur), soit environ 6,50 $ par jour.
...
function fallbackReply(message) {
  ...
  if (/prix|tarif|combien|co[uû]t|\$|199|6[,.]?50/.test(t)) {
    return 'Le forfait Pro est à 199 $/mois — environ 6,50 $ par jour — avec 14 jours d\'essai et sans contrat. Garantie 30 jours : si on rattrape pas au moins un client, on rembourse. Inscription : /signup.html?plan=pro';
  }
  if (/garantie|rembours/.test(t)) {
    return 'Garantie 30 jours : si aucun client n\'est rattrapé dans les 30 premiers jours payants, on rembourse sur demande. Essai 14 jours avant ça. Inscription : /signup.html?plan=pro';
  }
  if (/essai|gratuit|14/.test(t)) {
    return 'Essai 14 jours : carte requise pour activer la ligne, premier prélèvement après l\'essai. Ensuite garantie 30 jours. Annulation en un clic. Inscription : /signup.html?plan=pro';
  }
  ...
  if (/podium|concurrent|compar/.test(t)) {
    return 'NoviaAI vise les PME québécoises : tout-en-un (ligne + SMS + inbox + IA) à 199 $/mois (~6,50 $/jour), en français, sans contrat, avec garantie 30 jours — souvent 3 à 4 fois moins cher que Podium.';
  }
