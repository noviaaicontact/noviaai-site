...
async function processInboundActions({ tenant, callerPhone, userMessage, aiReply, history, calendarBooking }) {
  ...
  const mustPingOwner = intent && intent.type === 'human_transfer';
  const shouldNotify = mustPingOwner || (!!(intent || complete) && revenueLead);
  ...
  if (shouldNotify) {
    ...
    try {
      if (source === 'appointment') {
        ...
        const why = intent?.reason === 'complaint' ? 'Client mécontent' : 'Demande client';
