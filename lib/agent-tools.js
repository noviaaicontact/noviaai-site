...
async function processInboundActions({ tenant, callerPhone, userMessage, aiReply, history }) {
  ...
  // Créer le RDV dès qu'une heure est acceptée — même sans alerte courriel.
  try {
    const { maybeCreateCalendarEvent, hasConnectedCalendar } = require('./calendar');
    if (await hasConnectedCalendar(tenant.id)) {
  ...
  // Notifier seulement si intent fort (RDV / humain) ou qualification complète.
  const shouldNotify = !!(intent || complete);
