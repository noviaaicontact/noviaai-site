...
const { decideOwnerNotify, sourceFromDecision } = require('./owner-notify');
...
const COMPLAINT_RE = /plainte|frustr|déçu|decu|fâch|faché|fach\b|colère|colere|énerv|enerv|inacceptable|scandale|rembours|arnaque|insatisf|pas content|pas satisfait|incompétent|incompetent|jamais rappel|toujours pas rappel|après-vente|apres-vente|je vais (laisser|mettre) un avis|1 étoile|une étoile/i;
...
function classifyIntent(message, aiReply, workflow) {
  ...
  if (COMPLAINT_RE.test(clientText)) {
    return { type: 'human_transfer', summary: message, reason: 'complaint' };
  }
  if (HUMAN_RE.test(clientText)) {
    return { type: 'human_transfer', summary: message, reason: 'human' };
  }
  if (CALLBACK_RE.test(clientText)) {
    return { type: 'human_transfer', summary: message, reason: 'callback' };
  }
...
async function processInboundActions({ tenant, callerPhone, userMessage, aiReply, history, calendarBooking }) {
  ...
  const decision = await decideOwnerNotify({
    tenant,
    callerPhone,
    userMessage,
    aiReply,
  ...
  if (!decision.notify) {
    return { qualificationData, workflow, notify: false };
  }

  const mapped = sourceFromDecision(decision, intent, workflow);
  ...
  try {
    if (source === 'appointment') {
      ...
      const why = mapped.reason === 'complaint' || decision.kind === 'complaint'
        ? 'Client mécontent'
        : 'Demande client';
      await sendHumanTransferAlert(tenant, callerPhone, why, summaryText, qualificationData, qualHtml);
  ...
  return { intent: source, leadId: lead?.id, qualificationData, workflow, complete, notify: true };
}
