...
async function extractQualificationFromHistory({ history, callerPhone, fields, workflow }) {
  ...
  const wfHint = wf === WORKFLOW_APPOINTMENT
    ...
    : 'Contexte: entreprise de service terrain (plombier, électricien, garage). Inclure adresse si mentionnée — ne pas demander l\'urgence.';
...
async function loadThreadQualification(tenantId, callerPhone) {
  ...
  const { data } = await db.from('sms_threads')
    ...
    .maybeSingle();
  const q = data && data.qualification_data;
