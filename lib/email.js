...
const { INBOUND_LABELS, SOURCE_LABELS } = require('./marketing-lead');

function isPlaceholderEmail(addr) {
  const s = String(addr || '').trim();
  if (!s) return true;
  return /@noviaai\.invalid$/i.test(s);
}

/** Courriel proprio pour les alertes (lead, vocal, RDV). Jamais une adresse bidon d'onboarding. */
function ownerAlertEmail(tenant) {
  const contact = String((tenant && tenant.contact_email) || '').trim();
  const email = String((tenant && tenant.email) || '').trim();
  if (!isPlaceholderEmail(contact)) return contact;
  if (!isPlaceholderEmail(email)) return email;
  return process.env.ADMIN_EMAIL || 'noviaai.contact@gmail.com';
}

function marketingAlertRecipients() {
...
async function sendEmail({ to, subject, html }) {
  ...
  const recipients = (Array.isArray(to) ? to : [to])
    .map((s) => String(s || '').trim())
    .filter((s) => s && !isPlaceholderEmail(s));
  if (!key || !recipients.length) {
    console.log('[email skip]', subject, '→', recipients.join(', ') || to);
    return { skipped: true };
  }
...
async function sendWelcomeEmail(tenant) {
  const to = ownerAlertEmail(tenant);
...
async function sendHostedSmsRequestEmail(tenant) {
  ...
  const clientEmail = ownerAlertEmail(tenant);
  if (clientEmail) {
    await sendEmail({
      to: clientEmail,
...
async function sendCaptureLeadAlert(lead) {
  const to = marketingAlertRecipients();
  const utm = lead.utm || {};
  const origin = SOURCE_LABELS[lead.source_channel] || lead.source_channel || 'Direct';
  const inbound = INBOUND_LABELS[lead.inbound_channel] || lead.inbound_channel || '—';
  const missed = QUALIF_LABELS.missed[lead.missed_calls_per_month] || lead.missed_calls_per_month || '—';
  const jobValue = QUALIF_LABELS.value[lead.avg_client_value] || lead.avg_client_value || '—';
  const utmLine = [utm.source, utm.medium, utm.campaign].filter(Boolean).join(' / ') || '—';
  const html = `
    <h2>Nouveau prospect (pub / réseaux)</h2>
    <p><strong>${esc(lead.business_name)}</strong> — ${esc(lead.first_name)}</p>
    <table style="border-collapse:collapse;font-size:14px">
      ${qualifRow('Téléphone', lead.phone)}
      ${qualifRow('Courriel', lead.email)}
      ${qualifRow('Reçoit ses demandes via', inbound)}
      ${qualifRow('Appels manqués / mois', missed)}
      ${qualifRow('Valeur d’un job', jobValue)}
      ${qualifRow('Source', origin)}
      ${qualifRow('UTM', utmLine)}
    </table>
    <p style="margin-top:18px">
      ...
      &nbsp;
      <a href="${esc(process.env.PUBLIC_BASE_URL || 'https://noviaai.ca')}/admin.html" style="background:#13325b;color:#fff;padding:11px 16px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block">Voir dans l'admin</a>
    </p>
  `;
  return sendEmail({
    to,
    subject: utm.campaign === 'plombier'
      ? `Prospect pub plombier — ${lead.business_name} (${lead.first_name})`
      : `Prospect pub — ${lead.business_name} (${lead.first_name})`,
    html,
  });
}

/** Nouveau prospect issu de /potentiel ou /decouvrir. */
async function sendMarketingLeadAlert(lead, estimate) {
  if (lead && lead.form_variant === 'capture') return sendCaptureLeadAlert(lead);
