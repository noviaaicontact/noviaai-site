...
async function sendTrialEndingEmail(tenant, opts = {}) {
  ...
  const amount = opts.amountLabel || '199 $ CAD';
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;max-width:560px;margin:0 auto;color:#0b1f3b">
      ...
      <p>Après cette date, votre forfait Pro (<strong>${amount}/mois</strong>) sera prélevé automatiquement sur la carte enregistrée — aucun autre geste requis pour garder votre ligne active.</p>
...
async function sendPaymentFailedEmail(tenant, opts = {}) {
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;max-width:560px;margin:0 auto;color:#0b1f3b">
      ...
      <p>Bonjour${tenant.business_name ? ` — <strong>${tenant.business_name}</strong>` : ''},</p>
      <p>Nous n'avons pas pu prélever votre forfait Pro (<strong>${amount}/mois</strong>). Votre ligne reste active temporairement, mais elle sera suspendue si le paiement n'est pas régularisé.</p>
      <p style="margin:24px 0">
        <a href="${manageUrl}" style="background:#c8f135;color:#0b1f3b;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block">
...
async function sendPaymentReceiptEmail(tenant, opts = {}) {
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;max-width:560px;margin:0 auto;color:#0b1f3b">
      ...
      <p>Bonjour${tenant.business_name ? ` — <strong>${tenant.business_name}</strong>` : ''},</p>
      <p>Nous confirmons la réception de votre paiement NoviaAI Pro : <strong>${amount}</strong>.</p>
      ${period}
